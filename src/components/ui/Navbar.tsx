...
export default function Navbar({ cartCount = 0 }: { cartCount?: number }) {
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center h-[62px] gap-3">
            ...
            {/* Location pill */}
            <button className="hidden md:flex items-center gap-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl px-3 py-2 transition-colors min-w-[160px] shrink-0">
              <MapPin size={14} className="text-green-500 shrink-0" />
              <div className="text-left">
                <div className="text-[9px] text-gray-400 font-semibold uppercase leading-none mb-0.5">
                  Delivering to
                </div>
            ...
            <Link
              href="/orders"
              className="hidden md:flex flex-col items-center gap-0.5 p-2 hover:bg-gray-50 rounded-xl transition-colors"
            >
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-1">
            ...
            <Link
              href="/orders"
              className="flex items-center gap-3 px-3 py-3 text-gray-700 hover:bg-gray-50 rounded-xl text-sm font-semibold"
            ...
            <div className="border-t border-gray-100 pt-3 mt-1">
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-wider px-3 mb-2">
                For Business
              </p>
              <Link
                href="/shop/dashboard"
                className="flex items-center gap-3 px-3 py-2.5 text-green-600 font-bold hover:bg-green-50 rounded-xl text-sm"
                onClick={() => setMenuOpen(false)}
              >
              ...
              <Link
                href="/delivery"
                className="flex items-center gap-3 px-3 py-2.5 text-blue-600 font-bold hover:bg-blue-50 rounded-xl text-sm"
                onClick={() => setMenuOpen(false)}
              >
                🛵 Delivery Partner
              </Link>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 slide-up"
        style={{ boxShadow: "0 -4px 24px rgba(0,0,0,0.08)" }}
      >
        <div className="flex items-center justify-around py-1.5 px-2" style={{ paddingBottom: "calc(0.375rem + env(safe-area-inset-bottom, 0px))" }}>
          {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                <div className="relative">
                  <Icon size={22} className={isActive ? "stroke-green-600" : ""} strokeWidth={isActive ? 2.5 : 1.8} />
                  {isActive && (
